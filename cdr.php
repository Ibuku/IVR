...
do {
    ...
    try {
        ...
        // log call
        $record = '[DATETIME:'.time().'][STATUS: Incoming Call][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Incoming Call by '.$agi->get_variable('CDR(src)')['data'].']';
    ...
    try {
        ...
        $record = '[DATETIME:'.time().'][STATUS: Advert Impression][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Impression by '.$agi->get_variable('CDR(src)')['data'].']';
        append_line_to_limited_text_file($record);
    ...
    if ($values) {
        // log subscription
        $record = '[DATETIME:'.time().'][STATUS: Subscribed][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Subscription by '.$agi->get_variable('CDR(src)')['data'].']';
        append_line_to_limited_text_file($record);
        ...
        if ($zero_result !== null && $zero_result == 0) {
            ...
            // record confirmation
            $record = '[DATETIME:'.time().'][STATUS: Confirmation][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Subscription Confirmation by '.$agi->get_variable('CDR(src)')['data'].']';
            append_line_to_limited_text_file($record);
                ...
                try {
                    ...
                    if (isset($info['http_code'])) {
                        ...
                        if ($code == 200) {
                            ...
                            $record = '[DATETIME:'.time().'][STATUS: Successful][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Subscription Successful by '.$agi->get_variable('CDR(src)')['data'].']';
                            append_line_to_limited_text_file($record);
                            ...
                            try {
                                ...
                                if (strtolower($output->msg) == "insufficient_balance") {
                                    $record = '[DATETIME:'.time().'][STATUS: Insufficient Balance][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Subscription Status by '.$agi->get_variable('CDR(src)')['data'].']';
                                    append_line_to_limited_text_file($record);
                                    $agi->stream_file("defaults/insufficient");
                                } else if (strtolower($output->msg) == "already_subscribed") {
                                    // already subscribed
                                    $record = '[DATETIME:'.time().'][STATUS: Already Subscribed][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Already Subscribed by '.$agi->get_variable('CDR(src)')['data'].']';
                                    append_line_to_limited_text_file($record);
                                    $agi->stream_file("defaults/already_subscribed");
                                }
                            } catch (Exception $e) {
                                // subscription failed
                                $record = '[DATETIME:'.time().'][STATUS: Subscription Failed][Advert: ][MSISDN:'.$agi->get_variable('CDR(src)')['data'].'][MSG: Advert Subscription Failure by '.$agi->get_variable('CDR(src)')['data'].']';
                                append_line_to_limited_text_file($record);
